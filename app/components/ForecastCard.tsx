...
export default function ForecastCard({ dayName, code, condition, min, max, selected, onSelect }: ForecastCardProps) {
  return (
    <button
      ...
      className={`w-full h-full flex flex-col items-center justify-between gap-3 p-4 py-5 rounded-3xl border cursor-pointer transition-shadow duration-150 ${
    ...
    >
      ...
      <div className="flex flex-col items-center gap-2.5">
        ...
        <div className="text-xs text-muted capitalize text-center leading-tight">{condition}</div>
      </div>
